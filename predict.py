...
def predict_sliding_window(model, initial_en_input, future_de_inputs, device, num_output_features, qv_count):
    model.eval()
    ...
    with torch.no_grad():
        for t in tqdm(range(num_pred_steps), desc="[策略: 滑動窗口] 預測中"):
            ...
            if t < num_pred_steps - 1:
                next_en_input_history = current_en_input[:, 1:, :]
                if qv_count > 0:
                    sv_part = single_step_prediction[:, :, qv_count:]
                else:
                    sv_part = single_step_prediction
                new_step_features = torch.cat([sv_part, single_step_de_input], dim=2)
                current_en_input = torch.cat([next_en_input_history, new_step_features], dim=1)
    
    return predictions


def predict_block_replacement(model, initial_en_input, future_de_inputs, device, config, qv_count):
    model.eval()
    
    W = config['window']['train_window_mins'] // config['window']['sampling_interval_min']
    num_pred_steps = future_de_inputs.shape[1]
    
    if num_pred_steps % W != 0:
        print(f"警告: 預測總步數 {num_pred_steps} 不是窗口大小 {W} 的整數倍。")
        num_pred_steps = (num_pred_steps // W) * W
        print(f"預測將只進行到 {num_pred_steps} 步 (最後一個完整窗口)。")
        future_de_inputs = future_de_inputs[:, :num_pred_steps, :]

    num_windows_to_predict = num_pred_steps // W
    ...
    with torch.no_grad():
        for i in tqdm(range(num_windows_to_predict), desc="[策略: 塊替換] 預測中"):
            ...
            if i < num_windows_to_predict - 1:
                if qv_count > 0:
                    sv_part = prediction_block[:, :, qv_count:]
                else:
                    sv_part = prediction_block
                current_en_input = torch.cat([sv_part, de_input_block], dim=2)
...
def main(config_path):
    ...
    qv_cols = [c for c in y_sv if c.startswith('QV')]
    encoder_cols = [c for c in en_mv_and_sv if c not in qv_cols]
    qv_count = len(qv_cols)
    config['data']['qv_cols'] = qv_cols
    config['data']['encoder_cols'] = encoder_cols
    config['data']['num_qv'] = qv_count
    config['data']['num_en_input'] = len(encoder_cols)
    config['data']['num_de_input'] = len(de_mv)
    config['data']['num_output'] = len(y_sv)
    
    print(f"Encoder 輸入特徵數 (MV+SV): {len(encoder_cols)} | QV 數: {qv_count}")
    ...
    # --- Step 3: 執行預測 ---
    print("\n[3/4] 執行長期滾動預測...")
    initial_history_np = df_z_test.iloc[0:W][encoder_cols].values
    ...
    if inference_strategy == 'block_replacement':
        predictions_z = predict_block_replacement(model, initial_en_input, future_de_inputs, device, config, qv_count)
    elif inference_strategy == 'sliding_window':
        predictions_z = predict_sliding_window(model, initial_en_input, future_de_inputs, device, config['data']['num_output'], qv_count)
    else:
