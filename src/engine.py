...
def step_wise_rolling_training_step(model, batch, criterion, device, loss_fraction=None, qv_count=None):
    ...
    for t in range(n_steps):
        ...
        single_step_mask = mask[:, t, :].unsqueeze(1)
        abs_err = torch.abs(single_step_prediction - single_step_target)
        masked_err = abs_err * single_step_mask
        denom = single_step_mask.sum() + 1e-8
        ...
        if t < n_steps - 1:
            ...
            if qv_count is not None and qv_count > 0:
                sv_part = single_step_prediction.detach()[:, :, qv_count:]
            else:
                sv_part = single_step_prediction.detach()
...
def step_wise_rolling_at_loss_step(model, batch, criterion, device, config):
    qv_count = config['data'].get('num_qv', 0)
    ...
    en_input_initial, de_inputs, targets, mask = batch
    """測試打印用"""
    # print("mask 的形狀", mask.shape)
    # print("mask 的總和", mask.sum())
    # print("mask 為 0 的數量", (mask==0).sum())
    # 将所有数据移动到指定设备
    current_en_input = en_input_initial.to(device) # 维度: (B, H, mv+sv_features)
    ...
    for i in range(num_windows):
        ...
        if i < num_windows - 1:
            ...
            # **这里没有滑动，是完整的替换。**
            # **关键：没有 .detach()**
            sv_part = prediction_block[:, :, qv_count:] if qv_count > 0 else prediction_block
...
def train_one_epoch(model, dataloader, optimizer, criterion, device, training_step_fn, config=None, loss_fraction=None):
    ...
    for batch in tqdm(dataloader, desc="Training Progress"):
        optimizer.zero_grad()
        if training_step_fn.__name__ == 'step_wise_rolling_training_step':
            loss = training_step_fn(model, batch, criterion, device, loss_fraction, qv_count)
        else:
            loss = training_step_fn(model, batch, criterion, device, config)
        loss.backward()
        optimizer.step()
