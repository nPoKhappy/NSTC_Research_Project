...
class MultiStepS2SDataset(Dataset):
    def __init__(self, df, en_cols, de_cols, target_cols, input_len_steps, pred_len_steps, qv_stride_map=None):
        ...
        Args:
            ...
            qv_stride_map (dict, optional): 代表每個目標變量的採樣間隔，僅在特定時間步驟有真實值，其餘時間步驟的值為 0。
        """
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        if self.qv_stride_map:
            # 序列內的相對時間索引 0..H_out-1
            rel_time = torch.arange(self.H_out)
            for col_idx, col_name in enumerate(self.target_cols):
                if col_name in self.qv_stride_map:
                    stride = self.qv_stride_map[col_name]
                    # 只有 t % stride == 0 視為採樣點，其餘 mask=0
                    valid = (rel_time % stride == 0)
                    mask[:, col_idx] = valid.float()
        # 返回單個張量，不再是列表
