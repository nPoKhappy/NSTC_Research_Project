...
def main(config_path):
    ...
    # 構建 QV / SV+MV 分類 (假設所有以 'QV' 開頭的是 QV)
    ...
    config['data']['num_qv'] = len(qv_cols)
    # 定義低頻採樣 stride 映射 (可改由配置檔讀入)
    qv_stride_map = {'QV1_AI01':10, 'QV2_AI02':15}
    config['data']['qv_stride_map'] = qv_stride_map
    # 將變量維度信息存回配置
    ...
    # 創建 Dataset (使用過濾後的 encoder_cols)
    train_ds = MultiStepS2SDataset(train_df, encoder_cols, de_mv, y_sv, W, H_out, qv_stride_map=qv_stride_map)
    val_ds = MultiStepS2SDataset(val_df, encoder_cols, de_mv, y_sv, W, H_out, qv_stride_map=qv_stride_map)
